...
To run everything automatically you could try the provided
script. First you copy it to a desired place and then:
...
It will try to download the libraries, and benchmark, build them,
execute and create the plots. The current sizes are very big for
simple quick tests, so reduce them as explained bellow.
...
100 series PCIe adaptor.  All the kernels use the same code and same
standard BLAS functions from Intel MKL\,2018.4.

The current number of nodes to run the benchmarks is 1 and 2 (-N
1,2). If you want to run in more nodes you need to change them in the
scrips.
...
outputs: \code{output\_matvec.txt}, \code{output\_matmul.txt}
...
calculate the FLOPS and generate the plots (\code{build.sh} tries to
call it directly):
