...
By default cmake will try to use the gcc compiler, to use icc it is possible to pass:
\begin{lstlisting}
cmake -DCCOMPILER=intel ..
\end{lstlisting}

By default cmake will try to build with the system Lapack library
...
\subsubsection{Running benchmarks interactively}
...
One example execution and it output may be:
\begin{lstlisting}
...
\subsubsection{Running benchmarks on a Slurm cluster}
We also provide the scripts to submit the jobs in a cluster with Slurm
in case your system has such a setup.
...
\begin{lstlisting}
./submiter_dim.sh -D 32768 -B 32,64 -N 1,2,3 -I 5 -R 3 -C 24 -o results exe1 exe2 exe3
\end{lstlisting}

This will submit 3 jobs for 1,2 and 3 nodes (-N) respectively. That
will run the executables: \code{exe1}, \code{exe2} and \code{exe3} on
a loop 3 times each each (-R) with 24 cores per process (-C) and will
create a directory \code{results} (-o) to save all the outputs. With
matrix dimension 32768 (-D), block sizes of 32 and 64 (-B) and for 5
iterations (-I).

This script also creates a file inside results
