...
\centerline{ \code{cat /proc/sys/vm/overcommit\_memory}}

If the value is not zero, it should be set to zero using:

\centerline{\code{echo 0 > /proc/sys/vm/overcommit\_memory}}
...
\centerline{\code{echo 0 > /proc/sys/kernel/randomize\_va\_space}}
...
\paul{The double hyphens look like em dashes. Maybe we need to use a different font? Some
of the lines (here and below) extend into the margins.}
...
\paul{Say explicitly where these scripts are.}
...
then you can execute \code{submiter\_dim.sh} \paul{should be submitter\_dim.sh?} instead of
...
\begin{lstlisting}
./submiter_dim.sh -N 1,2 -R 3 -C 24 -D 32768 -B 32,64 -I 5 \
